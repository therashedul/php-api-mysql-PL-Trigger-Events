...
CREATE DEFINER=`root`@`localhost` PROCEDURE `update_emp` (IN `p_id` INT(10), IN `p_name` VARCHAR(200), IN `p_skills` VARCHAR(231), IN `p_address` VARCHAR(200), IN `p_designation` VARCHAR(200), IN `p_age` VARCHAR(200))  BEGIN
 UPDATE emp
 SET
 name = p_name, skills=p_skills, address = p_address, designation=p_designation, age=p_age WHERE id = p_id;
 END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `view_data` (INOUT `names` VARCHAR(255), INOUT `ages` VARCHAR(255))  BEGIN
select * from emp where name=names and age=ages;
END$$
...
INSERT INTO `emp` (`id`, `name`, `skills`, `address`, `designation`, `age`, `dlt_id`, `d_date`) VALUES
(1, 'sorna', 'cook', 'Dhaka', 'house wife', 21, NULL, '2020-02-16 05:10:29'),
(3, 'rasel karim', 'php', '', 'Developer', 23, NULL, '2020-02-16 10:55:14'),
(4, 'rasel', 'php', '', 'Developer', 32, 2, '2020-02-16 06:16:40');
...
(4, 'rasel', '32', 'php', 'Developer', '', 4, '2020-02-16 11:18:00');
...
(3, 'rasel', '32', 'php', '', 'Developer', 4, '2020-02-16 06:17:00');
...
(2, 'rasel', '32', 'php', 'Developer', '', 4, '2020-02-16 12:16:40'),
